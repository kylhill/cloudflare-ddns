...
records_response() {
    local total="$1" result="$2"
    jq -nc --argjson result "$result" --argjson total "$total" \
        '{success:true, result:$result, result_info:{total_count:$total}}'
}
...
if [[ "$method" == GET && "$url" == */dns_records ]]; then
    case "${TEST_SCENARIO:-}" in
        ...
        get_failure)
            ...
            ;;
        duplicate)
            records_response 2 "[$(json_record A1 198.51.100.42 false 3600)]"
            ;;
        ttl60)
        ...
        https_preserve)
            if [[ "$type" == A ]]; then
                ...
                records_response 1 "[$(json_https_record H1 'alpn="h3,h2" port="8443" ech="abc" ipv4hint="198.51.100.1" ipv6hint="2001:db8::1"' 3600)]"
            else
                records_response 0 '[]'
            fi
            ;;
...
expect_failure "duplicate count uses result_info total_count" duplicate -4 example.com
assert_contains "$TEST_TMP/duplicate count uses result_info total_count.out" "Found 2 A records"
